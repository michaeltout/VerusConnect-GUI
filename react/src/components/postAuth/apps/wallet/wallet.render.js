  ...
  const coinBalance = balances[coinObj.id]
    ? balances[coinObj.id].native.public.confirmed +
      (balances[coinObj.id].native.private.confirmed
        ...
        : 0)
    : "-";
  const balanceFiat = coinFiatRate && coinBalance !== '-' ? (coinFiatRate * coinBalance).toFixed(2) : '-'
  ...
  return (
    <button 
      ...
      <div
        ...
        <div className={`card ${isActive ? 'active-card' : 'border-on-hover'}`} style={WalletStyles.cardInnerContainer}>
          <div
            ...
            <div>
              <div
                ...
                <img
                  src={`assets/images/cryptologo/${coinObj.mode === ETH ? ETH : 'btc'}/${coinObj.id.toLowerCase()}.png`}
                  width="25px"
                  height="25px"
                />
                <h4
                  style={WalletStyles.cardCoinName}>
            ...
            <div style={{ paddingTop: 6 }}>
              <h6
                ...
                style={WalletStyles.fiatValue}>
                { `${balanceFiat}  ${fiatCurrency}` }
              </h6>
              <h5
                className="text-right"
                style={WalletStyles.balance}>
                { `${isNaN(coinBalance) ? coinBalance : Number(coinBalance.toFixed(8))} ${coinObj.id}` }
              </h5>
            </div>
          </div>
        </div>
